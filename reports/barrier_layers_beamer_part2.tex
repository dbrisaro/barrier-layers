...
%% ── ILD spread entre métodos ─────────────────────────────────────────────────
\begin{frame}{Dispersión de ILD entre los 6 métodos}
\begin{center}
\fullimg{../displays/ild_spread_maps.png}
\end{center}
\captext{Izq.: desviación estándar de la ILD media temporal entre los 6 métodos. Der.: rango (máx $-$ mín).
  La mayor dispersión aparece en el Océano Austral y en corrientes de borde oriental,
  donde la termoclina es débil y los métodos divergen.}
\end{frame}

%% ── CV de ILD + scatter ──────────────────────────────────────────────────────
\begin{frame}{¿Dónde importa más la elección del método?}
\begin{center}
\fullimg{../displays/ild_spread_cv_scatter.png}
\end{center}
\captext{Izq.: coeficiente de variación (CV $=$ std/media) de ILD --- proxy de ambigüedad del método.
  Der.: scatter ILD$_{\text{std}}$ vs ILD$_{\text{media}}$ coloreado por banda latitudinal.
  El Océano Austral (azul) concentra la mayor incertidumbre relativa.}
\end{frame}

\sectionframe{Tendencias 1993--2024}

%% ── Trend ILD / MLD / BLD global ────────────────────────────────────────────
\begin{frame}{Tendencia de ILD, MLD y BLD (1993–2024)}
\begin{center}
\fullimg{../displays/trends_ild_mld_bld_global.png}
\end{center}
\captext{Tendencia lineal (m/década) para el método gradiente $-0.1$\,°C/m.
  Panel A: ILD se profundiza en los trópicos.
  Panel B: MLD se superficializa (calentamiento superficial).
  Panel C: BLD aumenta por ambos efectos combinados.
  Punteado $=$ $p \geq 0.05$.}
\end{frame}

%% ── Trend ILD por método ─────────────────────────────────────────────────────
\begin{frame}{Tendencia de ILD: comparación de los 6 métodos}
\begin{center}
\fullimg{../displays/trends_ild_per_method.png}
\end{center}
\captext{Escala de color compartida entre los 6 paneles.
  Los métodos de gradiente (fila superior) muestran patrones consistentes.
  Los métodos de diferencia (fila inferior) presentan mayor señal en el Warm Pool del Pacífico.
  En el Océano Austral el signo del trend no es consistente entre métodos.}
\end{frame}

%% ── Trend regional: ILD / MLD / BLD ─────────────────────────────────────────
\begin{frame}{Tendencias regionales: ILD, MLD y BLD}
\begin{center}
\fullimg{../displays/trends_ild_mld_bld_regional.png}
\end{center}
\captext{Series anuales 1993--2024 para 5 regiones (filas) × 3 variables (columnas).
  En el Atlántico Tropical y el Océano Austral el trend de BLD está dominado por la superficialización de la MLD.
  En el Warm Pool del Pacífico Occidental tanto ILD como MLD contribuyen al aumento de la BLD.}
\end{frame}
